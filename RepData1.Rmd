...
The 5-minute interval, starting `r names(subsetmax)`  minutes into the day, is when the maximum number of steps (`r as.integer(subsetmax)`) is observed. 
...
The mean and median of the total number of steps taken per day using modified dataset are now equal at `r mean(totalstepsdailywithfill)`. A horozontal line has been added to both this histogram and the earlier histogram shown in section 1 above to help make clear how the filling action has modified the original dataset.  We see that the charts are identical apart from one bucket (corresponding to the mean of the total daily steps).
